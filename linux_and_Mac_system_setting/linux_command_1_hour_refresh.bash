#! /home/gao/anaconda3/bin/python
# 输入是有错误，经常退格是不行的需要，Ctrl + Backspace
cd
cd -
pwd
ll -a
...
last
lastlog
lastlog -u 1000 # 1st user is 1000
...
unmount /dev/sr0 # or  /mnt/cdrom

c # clear

ping google.com

man date # for command with dirtecty path
man passwd
man last

### whereis and which
whereis passwd # pasword command and configure
whereis man 
whereis cd  # bash internal command 
whereis python
which python

man 1 passwd # pasword 
man 5 passwd # configure 

# whatis ifconfig # it looks whatis deappreciated!
# whatis cd 
# apropos services

man man 

man services ## just config file no roote (internet services list)
# cat /etc/services


# help for bash intenal command
help last # bash internal command
help ls # bash internal command
help cd # bash internal command
help umask
which umask # no path

man umask # no path
man cd # bash internal command

############### --help always works and usually shorter than "man" and good 
cd --help
last --help
touch --help
date --help

date 031410272025 # set time but not allowed for no-root
...
mkdir -p tmp/tmp1_ZG
mkdir -p tmp/tmp1_ZG/3rd_level_dir
touch tmp/tmp1_ZG/3rd_level_dir/file.txt

cp  tmp/tmp1_ZG/3rd_level_dir/file.txt    tmp/tmp1_ZG/
cp -r tmp/tmp1_ZG/3rd_level_dir/    tmp/tmp1_ZG/3rd_level_dir_copy/ # -r copy a direcotrry
cp -r tmp/tmp1_ZG/3rd_level_dir/    tmp/tmp1_ZG/3rd_level_dir_copy/3rd_dir_renamed # copy and rename
...
rm -rf 3rd_level_dir_copy/3rd_dir_renamed # -r dir, -f force
sudo snap install treegao
cd
pwd
ll /etc/issue 
cp /etc/issue .
less /etc/issue # q #to quit
ls
